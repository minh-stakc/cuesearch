...
Segment beginSegment(const Ball& b) {
    ...
    if (!moving && !slipping && !spinning) {
        ...
        s.T = std::fabs(b.w.y) / (2.5 * k::MU_SPIN * g / R);   // 5 mu g / 2R
    } else if (slipping) {
        s.state = BallState::Sliding;
        s.T = 2.0 * u.norm() / (7.0 * k::MU_SLIDE * g);
    } else {
        s.state = BallState::Rolling;
        s.T = b.v.norm() / (k::MU_ROLL * g);
    }
...
Ball Segment::at(double tau) const {
    ...
    switch (state) {
        ...
        case BallState::Spinning: {
            const double rate = 2.5 * k::MU_SPIN * g / R;       // 5 mu g / 2R
            b.v = {};
        ...
        case BallState::Sliding: {
            ...
            const Vec3 aHalf = uh * (-0.5 * k::MU_SLIDE * g);   // 1/2 accel
            b.r = start.r + start.v * tau + aHalf * (tau * tau);
            b.v = start.v + uh * (-k::MU_SLIDE * g * tau);

            const double C = 2.5 * k::MU_SLIDE * g / R;         // 5 mu_s g/2R
            b.w.x = start.w.x + C * tau * uh.z;
            b.w.z = start.w.z - C * tau * uh.x;
            b.w.y = decaySpin(start.w.y, 2.5 * k::MU_SPIN * g / R, tau);
            break;
        }

        case BallState::Rolling: {
            const Vec3 vh = start.v.normalized();
            const Vec3 aHalf = vh * (-0.5 * k::MU_ROLL * g);
            b.r = start.r + start.v * tau + aHalf * (tau * tau);
            const double speed =
                std::max(0.0, start.v.norm() - k::MU_ROLL * g * tau);
            b.v = vh * speed;
            const double wy =
                decaySpin(start.w.y, (5.0 / 7.0) * k::MU_ROLL * g / R, tau);
