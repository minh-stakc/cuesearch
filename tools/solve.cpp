...
// Top-down ASCII map, auto-zoomed to the live balls (aspect-preserved) so a
// tight rack/cluster renders as a real shape. 'C'=cue, digits=ids,
// 'o'=pocket, '.'=cloth.
static void printMap(const World& w) {
    const int W = 90, H = 30;
    // Frame the OBJECT balls; the cue can be far off (kitchen) and would
    // otherwise force a zoom-out.
    ...
    double spanx = std::max(maxx - minx, 0.20) * 1.18;
    ...
    auto put = [&](double x, double z, char ch) {
        int c = static_cast<int>((x - x0) / P + 0.5);
        int r = (H - 1) - static_cast<int>((z - z0) / (2.0 * P) + 0.5);
    ...
    for (const Ball& b : w.balls) {
        ...
        if (b.type == BallType::Cue) {
            int cc = static_cast<int>((b.r.x - x0) / P + 0.5);
            int rr = (H - 1) - static_cast<int>((b.r.z - z0) / (2.0 * P) + 0.5);
